...
export function addSummaryToContext(route: any, data: any, dispatch: any) {
    route.sections.forEach((section: {travelSummary: {length: string, duration: string}}) => {
      if(data) {
        dispatch({type: Action.Add, payload: {startPlace: data[0].items[0], stopPlace:  data[1].items[0], distance: section.travelSummary.length, duration: section.travelSummary.duration}} )
